...
export type CourseWorkspaceShellProps = {
  children: ReactNode;
  viewerName: string;
...
function isNavItemActive(pathname: string, href: string, label: string) {
  // "Mi campus" only active on exact /mis-cursos
  if (label === "Mi campus") {
    return pathname === "/mis-cursos";
  }
...
function SidebarContent({
  ...
  return (
    <div className="flex h-full flex-col">
      {/* Brand — matches StudentShell */}
      <div className="px-6 pb-5 pt-8">
        <Link className="block" href="/mis-cursos" onClick={onNavigate}>
      ...
      {/* Main nav */}
      <nav className="flex-1 space-y-1 px-4" aria-label="Navegación campus">
        {navItems.map((item) => {
          const isActive = !item.disabled && isNavItemActive(pathname, item.href, item.label);
  ...
  return (
    <div className="min-h-screen lg:grid lg:grid-cols-[260px_minmax(0,1fr)]">
      ...
      <div className="flex min-h-screen flex-col">
        {/* Topbar */}
        <header className="sticky top-0 z-20 border-b border-[rgba(28,47,67,0.08)] bg-[rgba(255,255,255,0.95)] backdrop-blur-md">
            ...
            <div className="flex min-w-0 items-center gap-2.5">
              ...
              {courseTitle ? (
                <div className="flex min-w-0 items-center gap-2">
                  <GraduationCap className="h-4 w-4 shrink-0 text-[var(--color-ink-soft)]" strokeWidth={2} />
