...
export const defaultCourseCategories: CommunityCategoryDefinition[] = [
  {
    ...
    title: "Anuncios",
    description:
      "Avisos importantes, recordatorios y mensajes del equipo docente.",
    sortOrder: 1,
  },
  {
    slug: "dudas",
    title: "Dudas",
    description:
      "Preguntas del alumnado y resolucion compartida de incidencias del contenido.",
    sortOrder: 2,
  },
  {
    slug: "tareas",
    title: "Tareas",
    description:
      "Aclaraciones sobre ejercicios, actividades y seguimiento del trabajo del curso.",
    sortOrder: 3,
  },
  {
    slug: "general",
    title: "General",
    description:
      "Conversacion abierta sobre el curso, reflexiones y debate del grupo.",
    sortOrder: 4,
  },
  {
    ...
    description:
      "Recursos extra, enlaces recomendados y materiales complementarios.",
    sortOrder: 5,
  },
];
...
export async function ensureActiveForumSpace(courseSlug: string) {
  ...
  return db.forumSpace.create({
    data: {
      ...
      courseSlug,
      editionNumber,
      editionLabel: `Edición ${editionNumber}`,
      status: "ACTIVE",
    },
  });
}
...
const getAccessSnapshotForCourse = cache(
  async (
    ...
    const [courseAssignment, editionAssignment, enrollments] =
      await Promise.all([
...
const getAccessSnapshot = cache(
  async (
    ...
    return getAccessSnapshotForCourse(
      userId,
      course.id,
      course.slug,
      userGlobalRole,
...
export async function getUserCourseSpaces(input: {
  ...
  const [courseAssignments, editionAssignments, enrollments, purchases] =
    await Promise.all([
  ...
  const relevantCourseIds = Array.from(
    new Set([
      ...
      ...editionAssignments.map(
        (assignment) => assignment.courseEdition.courseId,
      ),
